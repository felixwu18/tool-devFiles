...
export class majorEditController extends ControllerBase {
	constructor() {
		super();
	}
  ...
  created() {
	 ...
	 */
  goBack() {
    this.$router.push({ path: "/workforceManagement/major", query: this.$route.query });
  }
 ...
 */
  holidayUpdate(meth) {
    this.http.LeaderShiftRequest[meth](this.tableData).then(res => {
      if (res.status == 200) {
