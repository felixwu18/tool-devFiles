...
export class FaxedReceivedListController extends ControllerBase {
  private temp = {
    style: require('../../style/faxManage/faxedReceivedList.less')
  };
  constructor() {
    super();
  }
  ...
  // 获取列表数据
  getListData() {
    this.http.MailListRequest.faxReceviedList(this.searchData).then((res) => {
      if (res.data) {
        //获取分页
        this.propRedData.total = res.data.total
        this.propRedData.pageSize = res.data.pageSize
        let propRedData = res.data.list.map((item, index) => {
          item.operate=this.btnGroupSe
       //   item.operate = [{ name: '删除', emit: 'faxDelete',type: 'danger', disabled: true ,expression:true}]
          return item
        })
        this.$set(this.propRedData, 'data', propRedData)
      }
    })
  }
  ...
  faxDelete(data) {
    console.log(data)
      this.$confirm('是否确认删除?',"提示",{
        ...
        this.http.MailListRequest.faxReceviedDelect(data.rowVal.faxSendId).then((res) => {
          if (res.status == 200) {
            this.getListData()
          } else {
            this.$message({
  ...
  /* author by chengyun 已收传真新增
   *  Modify by
   */
  controlAdd(){
    // /addressBookManage/deliverFaxAdd
    this.$router.push({path:'/addressBookManage/deliverFaxAdd',query:{title:'已收传真'}})
