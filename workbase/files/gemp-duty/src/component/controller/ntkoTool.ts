...
export class NtkoTool extends ControllerBase {
    constructor(){
        super()
    }
    ...
    mounted(){
        let id =  this.$route.fullPath.split("=")[1].split("&")[0]
        let type =  this.$route.fullPath.split("=")[2]
        this.GetDocFunc(id,type)
    }
    ...
    private TANGER_OCX_actionURL; //For auto generate form fields
    private TANGER_OCX_OBJ; //The Control
    // NTKO保存路由
    private action_url:string = baseUrl + '/api/gemp/duty/info/file/102/'


     //获取word文件路径
     GetDocFunc(id ,type) {
         console.log("发送请求")
         this.http.GempInfoBaseRequest.getDocInfo({id,type}).then(res => {
             if(res.status == 200){
    ...
    SaveToLocal(){
        ...
        //this.TANGER_OCX_OBJ. SaveToLocal ("c:\\test.doc",false)
    }
    ...
    TANGER_OCX_SaveDoc = function(filename,plantitle){
        ...
        try
        {
            ...
            (
                this.doc_url,  //此处为uploadedit.asp
