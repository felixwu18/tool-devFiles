...
//值班管理左侧机构树的组件
export class SearchTreeController extends ControllerBase {
  constructor() {
    super()
  }

  private temp = {
    style: require('../style/searchTree.less')
  }
  ...
  // 默认选中值
  //参数配置  //参数配置  1.接口需要入参时，加上 setParameters 参数;2.操作机构树按钮时，入参时加上 buttonShow；其目的做对应功能需求判断
  @Prop() configtree
  ...
  async getTreeData(method) {
    let params = this.configtree && this.configtree.setParameters ? this.configtree.setParameters : null
    let res = await this.http.TreeNode[method](params)
    ...
    this.lodeComplete(this.treeData)
