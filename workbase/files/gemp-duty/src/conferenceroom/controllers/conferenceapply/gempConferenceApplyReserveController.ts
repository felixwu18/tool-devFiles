...
import vue from 'vue';
//查看页面
export class GempConferenceApplyReserveController extends ControllerBase {
  constructor() {
    super();
  }
  ...
  private temp = {
    style: require('../../style/conferenceapply/gempConferenceApplyReserve.less'),
  };
  ...
  async saveData() {
    ...
    const res = await this.http.ConferenceroomapplyRequest.reserveGempConferenceApply(
      formData
    );
    if (res.status === 603) {
  ...
  //退回该申请单
  async back() {
    const res = await this.http.ConferenceroomapplyRequest.backGempConferenceApply(
      this.$route.query.id
    );
    if (res.status === 603) {
      this.$message.warning(res.msg);
