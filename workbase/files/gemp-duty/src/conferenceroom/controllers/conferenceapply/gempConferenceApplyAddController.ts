...
import vue from 'vue';
//查看页面
export class GempConferenceApplyAddController extends ControllerBase {
  constructor() {
    super();
  }
  ...
  private temp = {
    style: require('../../style/conferenceapply/gempConferenceApplyAdd.less'),
  };
  ...
  private ruleForm: any = {
    ...
    participantNum: '', //参会人数
    enablingMultimedia: '1', //是否启用多媒体   1启用 0不启用
    usingLeader: '', //使用领导
    tenantId: '', //租户id
    reserveName: '', //预定人
    reservePhone: '', //预定人电话
    ...
    remarks: '', //备注
    operatorId: '', //经办人ID
    operatorName: JSON.parse(sessionStorage.getItem('role')).name, //经办人姓名
    operatorPhone: JSON.parse(sessionStorage.getItem('role')).cellphone, //经办人电话
  ...
  async saveData() {
    ...
    const res = await this.http.ConferenceroomapplyRequest.saveGempConferenceApply(
      formData
    );
    if (res.status === 603) {
