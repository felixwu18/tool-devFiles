...
export class LowerHairIssueCopyController extends ControllerBase {
  private temp = {
  ...
  addReportInfo() {
    ...
    this.$refs.simpleTable['validate'](valid => {
      if (!valid) {
        ...
        this.http.DocumentHandleRequest.receipIssueAdd(newData).then(res => {
          if (res.status == 200) {
            this.$message({
              type:"success",
              message:"保存下发公文草稿成功"
  ...
  private addLower(el: string, name: string) {
    this.$refs.simpleTable['validate'](valid => {
      if (!valid) {
        ...
        this.dialogConfig['viewDialog'] = true;
        this.formdata['downSendingId'] = ''
        this.formdata['publicId'] = ''
        this.formdata['detailLinkId'] = ''
