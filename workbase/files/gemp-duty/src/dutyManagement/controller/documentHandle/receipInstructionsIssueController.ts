...
export class ReceipInstructionsIssueController extends ControllerBase {
  private temp = {
  ...
  getListData() {
    if (this.editId) {
      ...
      this.http.DocumentHandleRequest.getdutyInformationDetail(publi).then(
        res => {
          if (res.status == 200) {
            res.data.reviewDate = new Date(res.data.reviewDate.replace(/\-/g,'/'))
            this.$set(this,'formdata',res.data)
            this.getSelectNodes(this.formdata)
          }
        }
      );
    }
    ...
    this.http.SelectNode.getUptypeList().then(res=>{
      if(res.status == 200) {
        ...
        if(res.data.length==0){
          this.instructionEvents[0]={
            documentTitle:data.reviewEvent,
            publicId:data.reviewEvent
          } 
        }
      }
    })
  ...
  addReportInfo() {
    ...
    this.$refs.simpleTable['validate'](valid => {
      if (!valid) {
        ...
        this.http.DocumentHandleRequest.receipInstructsIssueEdit(newData).then(
          res => {
            if (res.status == 200) {
              this.$message({
                type:"success",
