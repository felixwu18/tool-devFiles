...
//值班记录单详情页
export class DutyRecordSheetDetailController extends ControllerBase {
  private temp = {
    style: require('../../style/dutyRecordSheet/dutyRecordSheetDetail.less')
  };
  constructor() {
    super();
  }
  ...
  onNotify() {
    ...
    let queryInfo = this.$route.query.dutyRecordId;
    let queryId = { id: queryInfo }
    this.http.DutyRecordSheet.dutySituationDetail(queryId).then(res => {
      if (res.status == 200) {
        // this.formdata.data = res.data.map(item => {
        //   switch (Number(item.recoredTypeCode)) {
        //     case 0:
        //       item.recoredType = '已办事项';
        //       break;
        //     case 1:
        //       item.recoredType = '待办事项';
        //       break;
        //     case 2:
        //       item.recoredType = '关注事项';
        //       break;
        //     default:
        //       break;
        //   }
        //   return item
        // })
        this.$set(this.formdata, 'data', res.data);
        this.format(this.formdata.data['recoredTypeCode'],"")
      }
    });
