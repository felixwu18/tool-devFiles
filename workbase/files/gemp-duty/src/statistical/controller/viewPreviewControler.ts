...
//   };
  ...
  private temp = {
    style: require('../style/viewPreview.less'),
  };
   // 值班信息文档地址
 ...
 private fullScreenStatus: boolean = false
  ...
  created(){
    // 获取值班信息文档
    // console.log(this.propdata.majorActivityId, 777)
    let param = {
        majorActivityId: this.propdata.majorActivityId,
        dailyActivityId: this.propdata.dailyActivityId,
        activityClass: this.propdata.activityClass,
    }
    if(this.propdata.type == 're') {
      this.http.SpecialCampaignRequest.getReViewDoc(param).then((res) => {
        if (res.status == 200) {
